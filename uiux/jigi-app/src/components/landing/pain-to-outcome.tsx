      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {painCards.map((card) => (
            <div
              key={card.pain}
              className="group flex flex-col items-center rounded-xl border border-border bg-card p-6 text-center shadow-sm transition-all duration-200 hover:shadow-md hover:border-primary/20"
            >
              {/* Pain */}
              <div className="rounded-lg bg-destructive/10 px-4 py-2.5 dark:bg-destructive/20">
                <p className="text-sm font-semibold text-destructive">{card.pain}</p>
              </div>

              <ArrowDown className="my-4 size-4 text-muted-foreground/50" />

              {/* Outcome */}
              <div className="rounded-lg bg-primary/10 px-4 py-2.5 dark:bg-primary/20">
                <p className="text-sm font-medium text-primary">{card.outcome}</p>
              </div>
            </div>
          ))}
        </div>
